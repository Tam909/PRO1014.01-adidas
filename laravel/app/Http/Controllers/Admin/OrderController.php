...
class OrderController extends Controller
{
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        // Chỉ cho phép tăng trạng thái +1
        if ($newStatus != $currentStatus + 1) {
            return back()->with('error', 'Chỉ được phép chuyển sang trạng thái tiếp theo.');
        }

        // Nếu chuyển sang trạng thái 3 (chuẩn bị giao): Trừ kho
        if ($newStatus == 3) {
            foreach ($order->orderItems as $item) {
                // Tìm variant theo product_id (vì chưa có variant_id)
                $variant = Varianti::where('id_pro', $item->product_id)->first();

                if (!$variant) {
                    return back()->with('error', 'Không tìm thấy phiên bản sản phẩm cho sản phẩm ID: ' . $item->product_id);
                }

                if ($variant->quantity < $item->quantity) {
                    return back()->with('error', 'Không đủ số lượng sản phẩm để giao cho sản phẩm ID: ' . $item->product_id);
                }
